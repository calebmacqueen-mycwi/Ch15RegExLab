...
class Validate {
    ...
    public function email($name, $value, $required = true) {
        ...
        if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
            $field->setErrorMessage("That not a valid email address");
        } else {
...
//                'Invalid domain name part.');
    ...
    public function password($name, $password, $required = true) {
        ...
        foreach($charClasses as $charClass) {
            $pw .= '(?=.*[' . $charClass . '])'; // working with optional characters does not like working with this syntax
            $valid .= $charClass;
        }
        $valid .= ']{8,}';
        $pw .= $valid . '$/';

        //echo $pw; // for debugging
        $pwMatch = preg_match('/^(?=.*[[:digit:]])(?=.*[[:upper:]])((\w)?)((\W)?)[[:digit:][:upper:](\w)?(\W)?]{8,}$/', $password);
    ...
    public function birthDate($name, $value) {
        $field = $this->fields->getField($name);
        $datePattern = '/^(0[1-9]|1[012])\/(0[1-9]|1[0-9]|2[0-9]|3[0-1])\/[1-9][[:digit:]]{3}?$/';
        $match = preg_match($datePattern, $value);
        if ( $match === false ) {
            $field->setErrorMessage('Error testing field.');
            return;
...
//        $year  = $dateParts[1];
        ...
        $now = new \DateTime();
        if ( $exp > $now ) {
            $field->setErrorMessage('That day has not yet occured. You may want to check your input.');
            return;
        } else {
        $field->clearErrorMessage();
        }
